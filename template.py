...
def load_template(template=None,block=False,variable=False,fix=False,form=False,overwrite=False):
	if template:
		blocks['']=template
		if overwrite:fix=True
		if fix:parse_fix()
		if block:parse_block()
		if variable:parse_variable(default=overwrite)
		if form:parse_form()
	return get_block('')
...
def parse_block(template=None):
	...
	def block(mm):
		...
		if m:
			...
			if m.group(3):return '{%s}' % m.group(1)
		return ''
	pattern='<!\s*(?:--(?:[^\-]|[\r\n]|-[^\-])*--\s*)>'
	...
	def dodos(template,vars):
		...
		while m:
			if m.group(1) in vars:
				s=vars[m.group(1)]
				if not m.group(1) in blocks:s=escape(s)
